 ...
 */
template <typename T>
bool Intersect( const Ray3<T>& pRay, const Plane3<T>& pPlane, glm::vec3& pPtIntersection, T& pDistance )
{
...
bool Intersect( const Plane3<T>& pPlane1, const Plane3<T>& pPlane2, Ray3<T>& pLine )
{
    ...
    T n01 = glm::dot(pPlane1.GetNormal(), pPlane2.GetNormal());
...
bool Intersect( const Ray3f& pRay, const BoundingBox& pBox )
{
...
bool IntersectTriangle( const glm::vec3& pVert0, const glm::vec3& pVert1, const glm::vec3& pVert2, const Ray3f& pRay )
{
    ...
    pvec = glm::cross(pRay.GetDirection(), edge2);
    ...
    // Calculate V parameter and test bounds.
    v = glm::dot(pRay.GetDirection(), qvec) * inv_det;
    if( v < 0.0 || u + v > 1.0 )
        return false;
