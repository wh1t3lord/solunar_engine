...
void D3D11RenderTarget::create(D3D11Device* device, const RenderTargetCreationDesc& renderTargetDesc)
{
	for (int i = 0; i < renderTargetDesc.m_textures2DCount; i++)
	{
		...
		D3D11_CHECK(device->getD3DDevice()->CreateRenderTargetView(d3dTexture, &renderTargetViewDesc, &renderTargetView));
		...
		D3D11_CHECK(device->getD3DDevice()->CreateShaderResourceView(d3dTexture, &shaderResourceViewDesc, &shaderResourceView));
	...
	if (renderTargetDesc.m_depthTexture2D)
	{
		...
		D3D11_CHECK(device->getD3DDevice()->CreateDepthStencilView(texture2D->getTexture(), &depthStencilViewDesc, &m_depthStencilView));
	}
}
